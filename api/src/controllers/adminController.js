...
const getClients = async (req, res) => {  
  try {  
    // Récupérer tous les clients avec leurs informations complètes  
    const clients = await Customer.find({ statut: 'ACTIF' })  
      .populate({  
    ...
    const formattedClients = clients.map(client => {  
      ...
      if (client.physical_user_id) {  
        clientData.user_info = {  
          ...
          ville: client.physical_user_id.ville, // AJOUTÉ
        ...
        clientData.user_info = {  
          ...
          patente: client.moral_user_id.patente, // AJOUTÉ  
          rc: client.moral_user_id.rc, // AJOUTÉ  
          ville_rc: client.moral_user_id.ville_rc, // AJOUTÉ  
          telephone_principal: client.moral_user_id.telephone_principal,  
          ville: client.moral_user_id.ville, // AJOUTÉ  
...
const getEmployees = async (req, res) => {  
  try {  
    // Récupérer tous les employés avec leurs informations complètes  
    ...
    const formattedEmployees = employees.map(employee => ({  
      ...
      user_info: {  
        ...
        ville: employee.physical_user_id.ville, // AJOUTÉ
...
const createUser = async (req, res) => {  
  try {  
    ...
    // Validation des régions autorisées (si fournie)  
    const validRegions = ['2 Mars', 'Maarif', 'Bir Anzarane', 'Boulevard al qods'];
    if (profile?.region_principale && !validRegions.includes(profile.region_principale)) {  
      return res.status(400).json({  
        success: false,  
        message: 'Région non autorisée. Les régions valides sont: ' + validRegions.join(', ')
      });  
    }  

    // Validation ville (si fournie)
    if (profile?.ville && profile.ville !== 'Casablanca') {  
      return res.status(400).json({  
        success: false,  
        message: 'Seule la ville Casablanca est autorisée'  
      });  
    }
    ...
    // Déterminer le rôle selon la fonction pour les employés  
    ...
    // Créer l'utilisateur principal avec statut ACTIF  
    ...
    if (type_personne === 'PHYSIQUE') {  
      ...
      const physicalUser = new PhysicalUser({  
        ...
        ville: profile.ville, // MODIFIÉ: Utiliser la valeur fournie
        region_principale: profile.region_principale, // MODIFIÉ: Utiliser la valeur fournie
  ...
  try {  
    ...
    if (physicalUser && profile) {    
      const allowedFields = [    
        ...
        'ville', // AJOUTÉ  
        'region_principale', 'date_naissance'    
      ];    
    ...
    if (moralUser && profile) {      
      const allowedFields = [      
        ...
        'telephone_principal', 'adresse_principale', 'ville', 'region_principale'      
      ];      
