    ...
    const formattedClients = clients.map(client => {
      ...
      if (client.physical_user_id) {
        clientData.user_info = {
          ...
          cin: client.physical_user_id.cin,
  ...
  try {
    ...
    const formattedEmployees = employees.map(employee => ({
      ...
      user_info: {
        ...
        cin: employee.physical_user_id.cin,
...
const createUser = async (req, res) => {
  try {
    ...
    // Créer l'utilisateur principal avec statut ACTIF (admin peut créer des utilisateurs actifs)
...
const updateUser = async (req, res) => {
  try {
    ...
    if (physicalUser && profile) {
      ...
      if (user.role_id.code === 'EMPLOYE' && employee_info) {  
